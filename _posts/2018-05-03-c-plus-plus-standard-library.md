...
static修饰的成员变量和成员函数都是全局的，所有类的实例都共用一份。static成员函数不能访问类的非静态数据成员、非静态函数成员。类的静态成员函数没有this指针。  
static成员变量不能在类的内部初始化，不占用类的实例对象的内存空间，存放在全局变量区。  

const成员函数不能修改成员变量的值，只能访问。  
const数据成员的初始化只能在类的构造函数的初始化列表中进行，初始换后不能修改，类的不同实例对象的值可以不同。
